...
export function trackCallClick(router: { push: (url: string) => void }, source?: string) {
  if (typeof window !== "undefined") {
    ...
    // Navigate the current window to the call thank-you page with source query param
    const url = source ? `/thank-you/call?source=${encodeURIComponent(source)}` : "/thank-you/call"
    router.push(url)
  }
}
...
export function trackWhatsAppClick(
  ...
  if (typeof window !== "undefined") {
    ...
    // Navigate the current window to the whatsapp thank-you page with source query param
    const url = source ? `/thank-you/whatsapp?source=${encodeURIComponent(source)}` : "/thank-you/whatsapp"
    router.push(url)
  }
}
 ...
 */
export function trackFormSubmit(router: { push: (url: string) => void }, source?: string) {
  if (typeof window !== "undefined") {
    pushToDataLayer("form_success", "form", source)
    const url = source ? `/thank-you/form?source=${encodeURIComponent(source)}` : "/thank-you/form"
    router.push(url)
  }
}
