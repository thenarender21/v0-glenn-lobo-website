...
export function FloorPlanShowcase({ onOpenContact }: { onOpenContact: (source: string) => void }) {
  ...
  const configs = {
    ...
    premium: {
      type: "2 BHK Premium XL (With Balcony)",
      carpet: "660 Sq.Ft.",
      builtup: "980 Sq.Ft.",
      living: "11'0\" x 16'6\"",
      master: "11'0\" x 12'0\"",
      bedroom2: "10'0\" x 10'6\"",
      kitchen: "8'0\" x 10'0\"",
      balcony: "5'0\" Wide Balcony",
    },
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 flex justify-center">
            <div className="relative w-full max-w-lg aspect-square rounded-2xl border border-border bg-card shadow-xl p-8 flex items-center justify-center overflow-hidden group">
              ...
              {/* Stylized SVG Floor Plan Schematic */}
              <svg viewBox="0 0 500 500" className="w-full h-full stroke-foreground/40 fill-none font-sans text-xs font-semibold select-none">
                {/* Outer frame */}
                <rect x="20" y="20" width="460" height="460" strokeWidth="4" rx="8" className="stroke-foreground/60" />
                
                {/* Rooms dividers */}
                {/* Living room (Bottom Left) */}
                <rect x="20" y="220" width="260" height="260" strokeWidth="2" />
                <text x="150" y="340" textAnchor="middle" className="fill-foreground font-medium text-sm">LIVING ROOM</text>
                <text x="150" y="360" textAnchor="middle" className="fill-muted-foreground text-[10px]">{current.living}</text>

                {/* Master Bed (Top Left) */}
                <rect x="20" y="20" width="260" height="200" strokeWidth="2" />
                <text x="150" y="110" textAnchor="middle" className="fill-foreground font-medium text-sm">MASTER BEDROOM</text>
                <text x="150" y="130" textAnchor="middle" className="fill-muted-foreground text-[10px]">{current.master}</text>

                {/* Bedroom 2 (Top Right) */}
                <rect x="280" y="20" width="200" height="230" strokeWidth="2" />
                <text x="380" y="125" textAnchor="middle" className="fill-foreground font-medium text-sm">BEDROOM 2</text>
                <text x="380" y="145" textAnchor="middle" className="fill-muted-foreground text-[10px]">{current.bedroom2}</text>

                {/* Kitchen (Bottom Right) */}
                <rect x="280" y="250" width="200" height="150" strokeWidth="2" />
                <text x="380" y="320" textAnchor="middle" className="fill-foreground font-medium text-sm">KITCHEN</text>
                <text x="380" y="340" textAnchor="middle" className="fill-muted-foreground text-[10px]">{current.kitchen}</text>

                {/* Bathrooms / Passage / Balcony */}
                {/* Master Bath */}
                <rect x="200" y="20" width="80" height="70" strokeWidth="1.5" className="fill-muted/20" />
                <text x="240" y="55" textAnchor="middle" className="fill-muted-foreground text-[9px]">TOILET</text>

                {/* Common Bath */}
                <rect x="280" y="400" width="100" height="80" strokeWidth="1.5" className="fill-muted/20" />
                <text x="330" y="445" textAnchor="middle" className="fill-muted-foreground text-[9px]">TOILET</text>

                {/* Balcony (if premium) */}
                {activeTab === "premium" && (
                  <>
                    <rect x="20" y="480" width="260" height="15" strokeWidth="1" className="fill-gold/10 stroke-gold/40" />
                    <text x="150" y="491" textAnchor="middle" className="fill-gold text-[8px] font-bold">BALCONY</text>
                  </>
                )}
              </svg>
          ...
          <div className="lg:col-span-5 text-left flex flex-col justify-center">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-semibold text-foreground">Master Bedroom</span>
                </div>
              ...
              <div className="flex items-center justify-between border-b border-border pb-3">
                <div className="flex items-center gap-3">
                  <Layout className="size-5 text-gold shrink-0" />
                  <span className="text-sm font-semibold text-foreground">Bedroom 2</span>
                </div>
                <span className="text-sm font-semibold text-foreground">{current.bedroom2}</span>
              </div>
            ...
            <Button
              onClick={() => onOpenContact("Floor Plan CTA - 2 BHK Campaign")}
              className="bg-gold hover:bg-gold-light text-charcoal h-14 font-semibold text-base shadow-lg transition-transform hover:scale-[1.02]"
            >
