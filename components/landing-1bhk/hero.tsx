...
export function Hero({ onOpenContact }: { onOpenContact?: () => void }) {
  ...
  const onSubmit = async (data: HeroFormData) => {
    ...
    try {
          ...
          records: [
            {
              fields: {
                ...
                "Lead Source": "1 BHK Hero Form",
                "Page URL": window.location.href,
                "Message": "Callback requested from 1 BHK campaign Hero form.",
              },
            },
          ],
        }),
      ...
      setTimeout(() => {
        ...
        trackFormSubmit(router, "1 BHK Hero Form")
      }, 1500)
    } catch (error) {
  ...
  return (
    <section className="relative flex min-h-[95vh] items-center justify-center overflow-hidden py-16 lg:py-24">
      <link 
        rel="preload" 
        href="/images/raunak-108/image_1_mobile.webp" 
        as="image" 
        media="(max-width: 767px)" 
      />
      <link 
        rel="preload" 
        href="/images/raunak-108/image_1_desktop.webp" 
        as="image" 
        media="(min-width: 768px)" 
      />
      {/* Background image & overlay */}
      <div
        className="absolute inset-0 bg-cover bg-center bg-no-repeat z-0 bg-[url('/images/raunak-108/image_1_mobile.webp')] md:bg-[url('/images/raunak-108/image_1_desktop.webp')]"
      >
            ...
            >
              <div className="mb-6 flex justify-start">
                <span className="rounded-full border border-gold/30 bg-charcoal/50 px-4 py-1.5 text-xs font-semibold tracking-widest text-gold backdrop-blur-md sm:text-sm">
                  EXCLUSIVE 1 BHK RESIDENCES
                </span>
              </div>
              
              <h1 className="text-balance text-4xl font-light leading-tight text-white sm:text-5xl md:text-6xl">
                Own Your <span className="font-medium text-gold">Dream 1 BHK</span> Home
              </h1>
              
              <p className="mt-6 max-w-2xl text-pretty text-lg text-white/80 sm:text-xl leading-relaxed">
                Experience premium living, excellent connectivity, modern amenities, and thoughtfully designed 1 BHK apartments built for comfort and convenience.
              </p>

              {/* Hero Features Grid */}
              <div className="mt-8 grid grid-cols-2 gap-4 max-w-lg">
                <div className="flex items-center gap-2 text-white">
                  <span className="text-gold font-bold">✓</span> Affordable Pricing
                </div>
                <div className="flex items-center gap-2 text-white">
                  <span className="text-gold font-bold">✓</span> Smart Layout Design
                </div>
                <div className="flex items-center gap-2 text-white">
            ...
            >
              ...
              <div className="flex w-full sm:w-auto gap-3">
                <Button
                  ...
                  onClick={() => trackCallClick(router, "1 BHK Hero Call")}
                >
                ...
                <Button
                  ...
                  onClick={() => trackWhatsAppClick(router, "https://wa.me/917972781688", "1 BHK Hero WhatsApp")}
                >
                  WhatsApp
                </Button>
              </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    <div className="bg-charcoal/80 border border-white/10 text-white/60 rounded-md h-11 px-3 flex items-center text-sm font-semibold select-none cursor-not-allowed">
                      1 BHK Premium Apartment (Pre-selected)
                    </div>
                  </div>
                    ...
                    <div className="rounded-lg bg-destructive/15 p-3 text-sm text-red-300 border border-destructive/20 space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={() => trackWhatsAppClick(router, "https://wa.me/917972781688", "1 BHK Hero Fallback")}
                        >
                          Chat on WhatsApp
                        </Button>
                        <Button
                          ...
                          onClick={() => trackCallClick(router, "1 BHK Hero Fallback")}
                        >
                          Call Us
                        </Button>
                      </div>
